...
export const CalendarGrid: React.FC<CalendarGridProps> = ({ currentDate, onDateClick, renderDay, onPrevMonth, onNextMonth }) => {
    ...
    return (
        <div className="bg-black/20 backdrop-blur-xl rounded-2xl border border-white/20 shadow-lg text-white p-4">
            <div className="flex justify-between items-center mb-4">
                <button onClick={onPrevMonth} className="p-2 rounded-full hover:bg-white/10" aria-label="Mes anterior">
                ...
                <button onClick={onNextMonth} className="p-2 rounded-full hover:bg-white/10" aria-label="Mes siguiente">
                    <ChevronDownIcon className="w-6 h-6 transform -rotate-90" />
                </button>
            </div>
            <div className="grid grid-cols-7 gap-1 text-center text-sm">
                {['Dom', 'Lun', 'Mar', 'Mié', 'Jue', 'Vie', 'Sáb'].map(day => <div key={day} className="font-semibold text-gray-400 py-2" aria-hidden="true">{day}</div>)}
