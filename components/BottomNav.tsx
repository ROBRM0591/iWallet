...
import { 
    ...
    SettingsIcon, ChatBubbleLeftRightIcon, ArrowUpIcon
} from './Icons';
...
export const BottomNav: React.FC = () => {
    ...
    return (
        <footer className="md:hidden fixed bottom-0 left-0 right-0 bg-white/80 dark:bg-black/30 backdrop-blur-lg shadow-lg z-40 border-t border-gray-200 dark:border-white/10">
            <nav className="grid grid-cols-5 gap-1 p-1">
                <NavItem to="/" icon={<DashboardIcon className="w-6 h-6"/>} label="Dashboard" />
                <NavItem to="/incomes" icon={<ArrowUpIcon className="w-6 h-6"/>} label="Ingresos" />
                ...
                <div className="relative flex justify-center more-menu-container">
                    <button onClick={() => setMoreOpen(v => !v)} className={`flex flex-col items-center w-full p-2 rounded-md text-sm font-medium transition-colors text-gray-500 dark:text-gray-300 hover:bg-black/5 dark:hover:bg-white/10`}>
                        <MoreIcon className="w-6 h-6"/>
                        <span className="text-xs mt-1">Más</span>
                    </button>
                    {moreOpen && (
                        <div className="absolute bottom-full mb-2 w-48 bg-white/80 dark:bg-gray-800/80 backdrop-blur-lg border border-gray-200 dark:border-white/10 rounded-lg shadow-xl p-2 z-50 right-0">
                            <NavLink to="/budget" onClick={() => setMoreOpen(false)} className="flex items-center gap-2 w-full text-left px-3 py-2 text-sm rounded-md text-gray-700 dark:text-gray-200 hover:bg-black/5 dark:hover:bg-white/10"><PiggyBankIcon className="w-5 h-5" /> Presupuesto</NavLink>
                            <NavLink to="/savings-goals" onClick={() => setMoreOpen(false)} className="flex items-center gap-2 w-full text-left px-3 py-2 text-sm rounded-md text-gray-700 dark:text-gray-200 hover:bg-black/5 dark:hover:bg-white/10"><SavingsGoalIcon className="w-5 h-5" /> Metas Ahorro</NavLink>
